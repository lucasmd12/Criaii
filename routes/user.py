...
from dependencies import get_db_manager, get_cache_service
...
async def get_current_user_id(authorization: Optional[str] = Header(None)):
    if not authorization or not authorization.startswith("Bearer "):
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Crachá de acesso (token Bearer) não encontrado.")
    token = authorization.split(" ")[1]
    user_id = verify_token(token)
    if not user_id:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Crachá de acesso (token) inválido ou expirado.")
    return user_id

# --- Rotas do Recepcionista (Atualizadas com DI e Lógica Híbrida) ---
...
async def register(
    ...
    return {"message": "Bem-vindo ao Alquimista Musical!", "user": user_dict, "token": token}
...
async def login(
    ...
    return {"message": "Login realizado com sucesso. Bom te ver de volta!", "user": user_dict, "token": token}
...
async def get_profile(
    ...
    print(f"✅ Recepcionista: Informações do cliente '{user_dict.get('username')}' encontradas no cofre.")
...
async def get_users():
    ...
    raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="A lista de todos os clientes é confidencial.")
